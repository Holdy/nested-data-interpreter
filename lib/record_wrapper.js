...
class record_wrapper {
    ...
    get_insert_values() {
        ...
        this.field_definitions.forEach(field_definition => {
            ...
            if (wrapper.literal_value || wrapper.literal_value === '') {
                ...
                const definition = pre_resolved_record.contains_field_definition(field_definition.name);
                if (!definition) {
                    throw new Error(`Attempted to find field: ${field_definition.name} but not present on ${wrapper.lookup_table_name}#${wrapper.lookup_record_reference}`);
                }
                ...
                if (!value && value != 0 && value != '') {
                    throw new Error(`Attempted to find field: ${field_definition.name} but value was empty on ${wrapper.lookup_table_name}#${wrapper.lookup_record_reference}`);
                } 
    ...
    set_resolved_value(name, value) {
        ...
        definition.value_wrapper.returned_value = value;
    }
}
